    ...
    [NfcModePoller] =
        {
            ...
            [NfcTechLegicPrime] = FuriHalNfcTechLegicPrime,
        },
    [NfcModeListener] =
        {
            [NfcTechIso14443a] = FuriHalNfcTechIso14443a,
...
static int32_t nfc_worker_listener(void *context) {
  ...
  while (true) {
    ...
    if (event & FuriHalNfcEventRxEnd) {
      ...
      furi_hal_nfc_listener_rx(instance->rx_buffer, sizeof(instance->rx_buffer),
                               &instance->rx_bits);
      bit_buffer_copy_bits(event_data.buffer, instance->rx_buffer,
                           instance->rx_bits);
...
static const NfcWorkerPollerStateHandler
    nfc_worker_poller_state_handlers[NfcPollerStateNum] = {
...
void nfc_config(Nfc *instance, NfcMode mode, NfcTech tech) {
  ...
  FuriHalNfcMode hal_mode =
      (mode == NfcModePoller) ? FuriHalNfcModePoller : FuriHalNfcModeListener;
  furi_hal_nfc_low_power_mode_stop();
...
NfcError nfc_listener_tx(Nfc *instance, const BitBuffer *tx_buffer) {
  ...
  FuriHalNfcError error = furi_hal_nfc_listener_tx(
...
NfcError nfc_iso14443a_poller_trx_custom_parity(Nfc *instance,
  ...
  do {
    ...
    bit_buffer_write_bytes_with_parity(tx_buffer, instance->tx_buffer,
                                       sizeof(instance->tx_buffer),
                                       &instance->tx_bits);
    error = furi_hal_nfc_iso14443a_poller_tx_custom_parity(instance->tx_buffer,
                                                           instance->tx_bits);
    if (error != FuriHalNfcErrorNone) {
    ...
    bit_buffer_copy_bytes_with_parity(rx_buffer, instance->rx_buffer,
                                      instance->rx_bits);
  } while (false);
...
NfcError nfc_poller_trx(Nfc *instance, const BitBuffer *tx_buffer,
  ...
  do {
    ...
    error = furi_hal_nfc_poller_tx(bit_buffer_get_data(tx_buffer),
...
NfcError nfc_iso14443a_listener_set_col_res_data(Nfc *instance, uint8_t *uid,
                                                 uint8_t uid_len, uint8_t *atqa,
                                                 uint8_t sak) {
  furi_check(instance);
...
NfcError nfc_iso14443a_poller_trx_sdd_frame(Nfc *instance,
  ...
  do {
    ...
    error = furi_hal_nfc_iso14443a_tx_sdd_frame(bit_buffer_get_data(tx_buffer),
                                                bit_buffer_get_size(tx_buffer));
    if (error != FuriHalNfcErrorNone) {
      FURI_LOG_D(TAG, "Failed in poller TX");
      ret = nfc_process_hal_error(error);
...
NfcError nfc_iso14443a_listener_tx_custom_parity(Nfc *instance,
  ...
  error = furi_hal_nfc_iso14443a_listener_tx_custom_parity(tx_data, tx_parity,
                                                           tx_bits);
  ret = nfc_process_hal_error(error);
...
NfcError nfc_felica_listener_set_sensf_res_data(
    Nfc *instance, const uint8_t *idm, const uint8_t idm_len,
    const uint8_t *pmm, const uint8_t pmm_len, const uint16_t sys_code) {
  furi_check(instance);

  FuriHalNfcError error = furi_hal_nfc_felica_listener_set_sensf_res_data(
      idm, idm_len, pmm, pmm_len, sys_code);
  instance->comm_state = NfcCommStateIdle;
  return nfc_process_hal_error(error);
}
...
NfcError nfc_legic_prime_poller_trx(Nfc *instance, uint8_t *trx_data) {
  ...
  do {
    ...
    instance->rx_bits = 0;
    error = furi_hal_nfc_poller_rx(trx_data, 0, &instance->rx_bits);
    if (error != FuriHalNfcErrorNone) {
      FURI_LOG_D(TAG, "Failed in poller RX");
      ret = nfc_process_hal_error(error);
      break;
