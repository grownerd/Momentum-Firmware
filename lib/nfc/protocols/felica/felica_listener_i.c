...
#define FELICA_SYSTEM_BLOCK_RO_ACCESS(data)                                    \
  (FELICA_MC_BYTE_GET(data, FELICA_MC_ALL_BYTE) == 0x00)
#define FELICA_SYSTEM_BLOCK_RW_ACCESS(data)                                    \
  (FELICA_MC_BYTE_GET(data, FELICA_MC_ALL_BYTE) == 0xFF)

#define FELICA_LISTENER_BLOCK_LIST_ITEM_SIZE_MIN                               \
  ((uint8_t)sizeof(FelicaBlockListElement))
...
static uint32_t felica_wcnt_get_max_value(const FelicaData *data) {
  if (!FELICA_SYSTEM_BLOCK_RO_ACCESS(data) &&
      !FELICA_SYSTEM_BLOCK_RW_ACCESS(data)) {
...
static bool felica_listener_block_list_item_validate_block_number(
...
static const FelicaBlockListElement *
felica_listener_block_list_iterate(FelicaListener *instance,
...
const FelicaBlockListElement *felica_listener_block_list_item_get_first(
    FelicaListener *instance, const FelicaListenerRequest *request) {
  furi_assert(instance);
  furi_assert(request);

  instance->request_size_buf =
      request->base.length - sizeof(FelicaListenerGenericRequest);
...
const FelicaBlockListElement *felica_listener_block_list_item_get_next(
    FelicaListener *instance, const FelicaBlockListElement *prev_item) {
  furi_assert(instance);
...
const FelicaListenerWriteBlockData *
felica_listener_get_write_request_data_pointer(
...
static bool felica_listener_check_write_request_data_size(
    const FelicaListener *const instance, const FelicaListenerRequest *request,
    const uint8_t fact_item_cnt) {
  uint8_t possible_data_size = fact_item_cnt * FELICA_DATA_BLOCK_SIZE;
  uint8_t fact_data_size = request->base.length -
                           sizeof(FelicaListenerGenericRequest) -
                           instance->block_list_size;
  return possible_data_size <= fact_data_size;
}

static bool felica_listener_test_block_list_size_bounds(
    const FelicaListenerGenericRequest *req) {
  bool valid = false;
  if (req->header.code == FELICA_CMD_READ_WITHOUT_ENCRYPTION) {
    valid = (req->header.block_count >= FELICA_LISTENER_READ_BLOCK_COUNT_MIN &&
             req->header.block_count <= FELICA_LISTENER_READ_BLOCK_COUNT_MAX);
  } else if (req->header.code == FELICA_CMD_WRITE_WITHOUT_ENCRYPTION) {
    valid = (req->header.block_count >= FELICA_LISTENER_WRITE_BLOCK_COUNT_MIN &&
...
static uint8_t
felica_listener_get_block_list_item_count_size(FelicaListener *instance,
                                               FelicaListenerRequest *request) {
  uint8_t list_size = 0;
  uint8_t item_cnt = 0;

  uint8_t max_item_cnt =
      (request->base.header.code == FELICA_CMD_READ_WITHOUT_ENCRYPTION)
...
bool felica_listener_check_block_list_size(FelicaListener *instance,
  ...
  uint8_t item_cnt =
      felica_listener_get_block_list_item_count_size(instance, request);
  valid = (item_cnt > 0);
  ...
  if (valid &&
      request->base.header.code == FELICA_CMD_WRITE_WITHOUT_ENCRYPTION) {
    valid &= felica_listener_check_write_request_data_size(instance, request,
                                                           item_cnt);
  }
...
bool felica_listener_check_idm(const FelicaListener *instance,
                               const FelicaIDm *request_idm) {
  furi_assert(instance);
...
void felica_listener_reset(FelicaListener *instance) {
  ...
  memcpy(instance->data->data.fs.mc.data, instance->mc_shadow.data,
...
static uint8_t felica_listener_get_block_index(uint8_t number) {
  ...
  } else if (number >= FELICA_BLOCK_INDEX_WCNT &&
             number <= FELICA_BLOCK_INDEX_STATE) {
...
static bool felica_get_mc_bit(const FelicaListener *instance,
                              uint8_t byte_index, uint8_t bit_number) {
...
static bool felica_block_requires_auth(const FelicaListener *instance,
                                       uint8_t command, uint8_t block_number) {
...
static bool felica_block_is_readonly(const FelicaListener *instance,
                                     uint8_t block_number) {
  bool readonly = false;
  if (block_number <= FELICA_BLOCK_INDEX_REG) {
    readonly = !felica_get_mc_bit(instance, FELICA_MC_SP_REG_ALL_RW_BYTES_0_1,
                                  block_number);
  } else if (((block_number == FELICA_BLOCK_INDEX_ID) ||
              (block_number == FELICA_BLOCK_INDEX_SER_C)) ||
             (block_number >= FELICA_BLOCK_INDEX_CKV &&
              block_number <= FELICA_BLOCK_INDEX_CK)) {
    const FelicaData *data = instance->data;
    readonly = FELICA_SYSTEM_BLOCK_RO_ACCESS(data);
  } else if ((block_number == FELICA_BLOCK_INDEX_MAC) ||
             (block_number == FELICA_BLOCK_INDEX_WCNT) ||
...
static bool felica_block_requires_mac(const FelicaListener *instance,
                                      uint8_t block_number) {
  bool need_mac = false;
  if (block_number <= FELICA_BLOCK_INDEX_REG) {
    need_mac = felica_get_mc_bit(instance, FELICA_MC_SP_REG_W_MAC_A_BYTES_10_11,
                                 block_number);
  } else if (block_number == FELICA_BLOCK_INDEX_CK ||
             block_number == FELICA_BLOCK_INDEX_CKV) {
...
static void
felica_handler_read_block(FelicaListener *instance, const uint8_t block_number,
  ...
  memcpy(&response->data[resp_data_index * FELICA_DATA_BLOCK_SIZE],
...
static void
felica_handler_read_all_zeros(FelicaListener *instance,
  ...
  memset(&response->data[resp_data_index * FELICA_DATA_BLOCK_SIZE], 0,
         FELICA_DATA_BLOCK_SIZE);
  response->header.length += FELICA_DATA_BLOCK_SIZE;
}

static void
felica_handler_read_mac_a_block(FelicaListener *instance,
                                const uint8_t block_number,
                                const uint8_t resp_data_index,
                                FelicaListenerReadCommandResponse *response) {
  if ((resp_data_index != response->block_count - 1) ||
      (resp_data_index == 0)) {
    felica_handler_read_all_zeros(instance, block_number, resp_data_index,
                                  response);
    instance->mac_calc_start = resp_data_index + 1;
  } else {
    felica_calculate_mac_read(
        &instance->auth.des_context, instance->auth.session_key.data,
    ...
    felica_handler_read_block(instance, block_number, resp_data_index,
                              response);
  }
}

FelicaCommanReadBlockHandler
felica_listener_get_read_block_handler(const uint8_t block_number) {
  FelicaCommanReadBlockHandler handler = felica_handler_read_block;

  if (block_number == FELICA_BLOCK_INDEX_RC ||
      block_number == FELICA_BLOCK_INDEX_CK) {
...
static bool
felica_validate_read_block_list(FelicaListener *instance,
  ...
  for (uint8_t i = 0; i < request->base.header.block_count; i++) {
    if (item->service_code != 0) {
      ...
      response->SF2 = 0xA7;
      return false;
    } else if (!felica_listener_block_list_item_validate_block_number(item) ||
               !felica_block_exists(item->block_number) ||
               (felica_block_is_readonly(instance, item->block_number) &&
                request->base.header.service_code !=
    ...
    } else if (felica_block_requires_auth(instance, request->base.header.code,
                                          item->block_number) &&
...
bool felica_listener_validate_read_request_and_set_sf(
  ...
  do {
    ...
    if ((request->base.header.code == FELICA_CMD_READ_WITHOUT_ENCRYPTION) &&
        (request->base.header.block_count <
             FELICA_LISTENER_READ_BLOCK_COUNT_MIN ||
         request->base.header.block_count >
             FELICA_LISTENER_READ_BLOCK_COUNT_MAX)) {
      resp_header->SF1 = 0xFF;
    ...
    if (!felica_validate_read_block_list(instance, request, resp_header))
      break;
...
static bool felica_validate_write_block_list(
  ...
  if (request->base.header.block_count ==
          FELICA_LISTENER_WRITE_BLOCK_COUNT_MAX &&
      items[1]->block_number == FELICA_BLOCK_INDEX_MAC_A) {
    write_with_mac = true;
  } else if (request->base.header.block_count <
                 FELICA_LISTENER_WRITE_BLOCK_COUNT_MIN ||
             request->base.header.block_count >
                 FELICA_LISTENER_WRITE_BLOCK_COUNT_MAX) {
    response->SF1 = 0x02;
    response->SF2 = 0xB2;
  ...
  for (uint8_t i = 0; i < request->base.header.block_count; i++) {
    ...
    if (item->service_code != 0) {
      ...
      felica_calculate_mac_write(
          &instance->auth.des_context, instance->auth.session_key.data,
          instance->data->data.fs.rc.data, data->blocks[1].data + 8,
          data->blocks[0].data, calculated_mac);

      if (!instance->rc_written ||
          (memcmp(calculated_mac, data->blocks[1].data, 8) != 0) ||
...
bool felica_listener_validate_write_request_and_set_sf(
  ...
  do {
    ...
    if ((request->base.header.code == FELICA_CMD_WRITE_WITHOUT_ENCRYPTION) &&
        (request->base.header.block_count < 0x01 ||
         request->base.header.block_count > 0x02)) {
      response->SF1 = 0xFF;
      response->SF2 = 0xA2;
      break;
    }
    ...
    if (!felica_validate_write_block_list(instance, request, data, response))
      break;
...
static void felica_handler_write_block(FelicaListener *instance,
                                       const uint8_t block_number,
                                       const FelicaBlockData *data_block) {
  uint8_t num = felica_listener_get_block_index(block_number);

  memcpy(&instance->data->data.dump[num * (FELICA_DATA_BLOCK_SIZE + 2) + 2],
         data_block->data, FELICA_DATA_BLOCK_SIZE);
}

static void felica_handler_write_rc_block(FelicaListener *instance,
                                          const uint8_t block_number,
                                          const FelicaBlockData *data_block) {
  felica_handler_write_block(instance, block_number, data_block);

  felica_calculate_session_key(
      &instance->auth.des_context, instance->data->data.fs.ck.data,
      instance->data->data.fs.rc.data, instance->auth.session_key.data);
  instance->rc_written = true;
}

static void felica_handler_write_reg_block(FelicaListener *instance,
                                           const uint8_t block_number,
                                           const FelicaBlockData *data_block) {
  UNUSED(block_number);
...
static void felica_handler_write_mc_block(FelicaListener *instance,
                                          const uint8_t block_number,
                                          const FelicaBlockData *data_block) {
  UNUSED(block_number);

  bool mc_bits_permission =
      felica_get_mc_bit(instance, FELICA_MC_SP_REG_ALL_RW_BYTES_0_1, 15);
  const FelicaData *data = instance->data;
  ...
  for (uint8_t i = 0; i < FELICA_DATA_BLOCK_SIZE; i++) {
    ...
    } else if ((i >= FELICA_MC_ALL_BYTE && i <= FELICA_MC_CKCKV_W_MAC_A) &&
               (mc_system_block_permission)) {
      instance->mc_shadow.data[i] = data_block->data[i];
    } else if ((i >= FELICA_MC_SP_REG_R_RESTR_BYTES_6_7 &&
                i <= FELICA_MC_STATE_W_MAC_A) &&
               (mc_bits_permission)) {
      instance->mc_shadow.data[i] |= data_block->data[i];
...
static void
felica_handler_write_state_block(FelicaListener *instance,
                                 const uint8_t block_number,
                                 const FelicaBlockData *data_block) {
  felica_handler_write_block(instance, block_number, data_block);
...
static void felica_handler_write_id_block(FelicaListener *instance,
                                          const uint8_t block_number,
                                          const FelicaBlockData *data_block) {
  UNUSED(block_number);
