...
void legic_prng_forward(int count) {
    ...
    lfsr.c += count;
    while (count--) {
        // According: http://www.proxmark.org/forum/viewtopic.php?pid=5437#p5437 <-- Dead link!
        // New Link: https://www.proxmark.io/www.proxmark.org/forum/viewtopic.php%3Fpid=5437.html
...
uint32_t legic_prng_get_count(void) {
    return lfsr.c;
}

uint8_t legic_prng_get_iv(void) {
    return lfsr.iv;
}

void legic_prng_backup(int count) {
    uint32_t c_tmp = lfsr.c;
    uint8_t iv_tmp = lfsr.iv;

    legic_prng_init(iv_tmp);
    legic_prng_forward(c_tmp - count);
}
